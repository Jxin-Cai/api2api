...
final class ConverterFieldMappingDescriptions {
    ...
    static {
        ...
        map.put(key(ProtocolType.CLAUDE_MESSAGES, ProtocolType.OPENAI_RESPONSES, ProtocolConversionDirection.REQUEST), List.of(
                ...
                mapping("content[].type=document", "input[].content[].type=input_file", "文档块转为文件输入；title/media_type 用于 filename 与 data URI，无法挂到 file 部分的 title/context 改为相邻 input_text", MappingLossiness.PARTIAL, "CONTENT_BLOCK", "RESHAPE"),
                mapping("content[].source.data", "input[].content[].file_data", "base64 文档包装为 data:{media_type};base64 写入 file_data", MappingLossiness.NONE, "CONTENT_BLOCK", "TRANSFORM"),
                mapping("content[].source.media_type", "input[].content[].filename / file_data", "MIME 类型写入 data URI 并推导文件扩展名", MappingLossiness.NONE, "CONTENT_BLOCK", "TRANSFORM"),
                mapping("content[].title", "input[].content[].filename", "文档标题映射为 filename；file_id/file_url 场景改为相邻 input_text", MappingLossiness.NONE, "CONTENT_BLOCK", "RESHAPE"),
                mapping("content[].context", "input[].content[].type=input_text", "文档上下文作为相邻文本块保留", MappingLossiness.NONE, "CONTENT_BLOCK", "RESHAPE"),
                mapping("content[].source.url", "input[].content[].file_url", "文档 URL 写入 file_url（不附带 filename，避免与 Responses 互斥字段冲突）", MappingLossiness.NONE, "CONTENT_BLOCK", "RESHAPE"),
                mapping("content[].source.file_id", "input[].content[].file_id", "文件 ID 直接映射", MappingLossiness.NONE, "CONTENT_BLOCK", "DIRECT"),
                mapping("content[].source.type=content", "input[].content[]", "内联 content 文档展开为 input_text/input_image", MappingLossiness.NONE, "CONTENT_BLOCK", "RESHAPE"),
                mapping("content[].type=image source.type=file", "input[].content[].file_id", "图片 Files API 引用映射为 input_image.file_id", MappingLossiness.NONE, "CONTENT_BLOCK", "RESHAPE"),
                mapping("content[].cache_control", "prompt_cache_key", "校验 Claude 缓存配置；显式断点当前关闭，依赖稳定 key 与上游隐式缓存", MappingLossiness.PARTIAL, "METADATA", "RESHAPE"),
                ...
                mapping("content[].is_error", "function_call_output.output", "失败结果添加明确的文本标记；保留多模态正文，不设置未完成 status", MappingLossiness.PARTIAL, "TOOL", "TRANSFORM"),
                mapping("tool_result.content[].type=search_result", "function_call_output.output[].input_text", "保留检索正文、标题和来源；不伪造结构化引用", MappingLossiness.PARTIAL, "TOOL", "RESHAPE"),
                mapping("tool_result.content[].type=browser_state", "function_call_output.output[].input_text", "浏览器 tabs 与 state_changes 序列化为模型可见文本", MappingLossiness.PARTIAL, "TOOL", "RESHAPE"),
                mapping("tool_result.content[].type=tool_reference", "tools[].defer_loading", "成功发现或历史已调用的函数立即加载，保留原名称与 schema", MappingLossiness.PARTIAL, "TOOL", "TRANSFORM"),
                ...
                unsupported("top_k", "Responses 无等价采样参数，请求明确失败", "MODEL"),
                unsupported("stop_sequences", "Responses 无等价停止序列参数，非空请求明确失败", "MODEL"),
                ...
                mapping("tool_choice", "tool_choice", "函数按名称选择；托管工具按真实类型使用 allowed_tools + required", MappingLossiness.NONE, "TOOL", "RESHAPE"),
        ...
        map.put(key(ProtocolType.OPENAI_RESPONSES, ProtocolType.CLAUDE_MESSAGES, ProtocolConversionDirection.RESPONSE), List.of(
                ...
                mapping("output[].type=reasoning", "content[].type=thinking", "外部推理状态经签名桥接为 thinking 块；桥接的 Claude 原生状态直接还原为签名 thinking/redacted_thinking", MappingLossiness.PARTIAL, "REASONING", "RESHAPE"),
                mapping("output[].type=compaction", "content[].type=thinking (opaque) + text", "压缩状态经签名桥接保留并输出可见摘要文本", MappingLossiness.PARTIAL, "REASONING", "RESHAPE"),
                mapping("output[].type=program(_output)", "content[].type=server_tool_use/code_execution_tool_result", "programmatic 执行项映射为 Claude 服务端工具块", MappingLossiness.PARTIAL, "TOOL", "RESHAPE"),
                mapping("status", "stop_reason", "状态映射为停止原因 (incomplete→max_tokens/refusal, 无终结消息→pause_turn)", MappingLossiness.NONE, "MESSAGE", "TRANSFORM"),
                ...
                mapping("usage.input_tokens_details.cache_write_tokens", "usage.cache_creation_input_tokens", "缓存写入 token 字段路径转换", MappingLossiness.NONE, "USAGE", "RENAME"),
        ...
        map.put(key(ProtocolType.OPENAI_RESPONSES, ProtocolType.CLAUDE_MESSAGES, ProtocolConversionDirection.REQUEST), List.of(
                mapping("input", "messages", "输入项转为 Claude 消息结构；工具配对与同角色合并按 Anthropic 约束规整", MappingLossiness.NONE, "MESSAGE", "RESHAPE"),
                mapping("input[].role", "messages[].role", "角色映射 (system/developer→system 提示)", MappingLossiness.NONE, "MESSAGE", "TRANSFORM"),
                mapping("input[].content", "messages[].content", "内容格式转换", MappingLossiness.NONE, "MESSAGE", "RESHAPE"),
                mapping("instructions", "system", "指令拼接进 system 提示词", MappingLossiness.NONE, "MESSAGE", "RESHAPE"),
                mapping("input[].content[].type=input_image", "content[].type=image", "data URI 拆解为 base64 source，URL 原样映射", MappingLossiness.NONE, "CONTENT_BLOCK", "TRANSFORM"),
                mapping("input[].content[].type=input_file", "content[].type=document", "file_id/file_url/file_data 映射为文档来源；data URI 拆出 media_type，filename 还原 title", MappingLossiness.NONE, "CONTENT_BLOCK", "RESHAPE"),
                mapping("input[].content[].file_id", "content[].source.file_id", "图片或文件 ID 映射为 source.type=file", MappingLossiness.NONE, "CONTENT_BLOCK", "DIRECT"),
                mapping("input[].type=function_call", "content[].type=tool_use", "function_call 历史项转为 tool_use 块", MappingLossiness.NONE, "TOOL", "RESHAPE"),
                mapping("input[].type=function_call_output", "content[].type=tool_result", "工具输出转为 tool_result 并与调用配对", MappingLossiness.NONE, "TOOL", "RESHAPE"),
                mapping("input[].type=custom_tool_call(_output)", "content[].type=tool_use/tool_result", "自由文本自定义工具经 {input} 包装往返", MappingLossiness.NONE, "TOOL", "RESHAPE"),
                mapping("input[].type=reasoning (bridged)", "content[].type=thinking/redacted_thinking", "encrypted_content 中桥接的 Claude 原生签名状态还原为签名 thinking 块；外部 OpenAI 状态无法伪造签名而被丢弃", MappingLossiness.PARTIAL, "REASONING", "TRANSFORM"),
                mapping("input[].type=compaction", "content[].type=compaction", "压缩摘要转为 Claude compaction 块", MappingLossiness.PARTIAL, "REASONING", "RESHAPE"),
                mapping("model", "model", "Direct passthrough", MappingLossiness.NONE, "MODEL", "DIRECT"),
                mapping("max_output_tokens", "max_tokens", "字段重命名；缺省时补 Claude 必填默认值", MappingLossiness.NONE, "MODEL", "RENAME"),
                mapping("temperature", "temperature", "Direct passthrough", MappingLossiness.NONE, "MODEL", "DIRECT"),
                mapping("top_p", "top_p", "Direct passthrough", MappingLossiness.NONE, "MODEL", "DIRECT"),
                mapping("tools", "tools", "function/custom/web_search/code_interpreter/mcp 工具定义转换", MappingLossiness.NONE, "TOOL", "RESHAPE"),
                mapping("tools[].type=mcp", "mcp_servers + mcp_toolset", "MCP 服务器定义与 allowlist/延迟加载映射", MappingLossiness.PARTIAL, "TOOL", "RESHAPE"),
                mapping("tools[].allowed_callers", "tools[].allowed_callers", "programmatic 调用者映射为 code_execution caller", MappingLossiness.NONE, "TOOL", "TRANSFORM"),
                mapping("tool_choice", "tool_choice", "auto/none/required/function 策略映射", MappingLossiness.NONE, "TOOL", "RESHAPE"),
                mapping("parallel_tool_calls", "tool_choice.disable_parallel_tool_use", "布尔语义取反", MappingLossiness.NONE, "TOOL", "TRANSFORM"),
                mapping("reasoning.effort", "thinking + output_config.effort", "effort 映射为 thinking budget_tokens 与 effort；none/minimal 关闭 thinking", MappingLossiness.PARTIAL, "REASONING", "TRANSFORM"),
                mapping("text.format (json_schema)", "output_config.format", "结构化输出 schema 映射", MappingLossiness.NONE, "MODEL", "RESHAPE"),
                mapping("metadata.user_id / safety_identifier / user", "metadata.user_id", "用户标识按优先级归一", MappingLossiness.PARTIAL, "METADATA", "TRANSFORM"),
                mapping("service_tier", "service_tier", "auto/default 映射；flex/priority/scale 无等价语义丢弃", MappingLossiness.PARTIAL, "METADATA", "TRANSFORM"),
                mapping("stream", "stream", "Direct passthrough", MappingLossiness.NONE, "STREAMING", "DIRECT"),
                mapping("provider-hosted history items", "(dropped)", "web_search_call/code_interpreter_call 等托管历史项无法由 Claude 重放，予以丢弃", MappingLossiness.LOSSY, "TOOL", "DROP"),
                unsupported("previous_response_id", "网关无服务端会话存储，无法解析引用", "METADATA"),
                unsupported("conversation", "网关无服务端会话存储，无法解析引用", "METADATA"),
                unsupported("prompt", "网关无服务端存储的 prompt 模板", "METADATA"),
                unsupported("background", "Claude Messages 无后台任务模式", "METADATA"),
                unsupported("input[].type=item_reference", "网关无服务端存储，无法解析 item 引用", "MESSAGE")
        ));
        ...
        map.put(key(ProtocolType.CLAUDE_MESSAGES, ProtocolType.OPENAI_RESPONSES, ProtocolConversionDirection.RESPONSE), List.of(
                ...
                mapping("content[].type=thinking", "output[].type=reasoning", "thinking 块转为 reasoning 项；原生签名经 encrypted_content 隧道往返", MappingLossiness.NONE, "REASONING", "RESHAPE"),
                mapping("content[].type=redacted_thinking", "output[].type=reasoning (encrypted)", "加密推理数据经 encrypted_content 隧道往返", MappingLossiness.NONE, "REASONING", "RESHAPE"),
                mapping("content[].type=compaction", "output[].type=compaction", "压缩摘要转为 Responses compaction 项", MappingLossiness.PARTIAL, "REASONING", "RESHAPE"),
                mapping("stop_reason", "status", "停止原因映射为状态；max_tokens/context 溢出产生 incomplete 终止事件", MappingLossiness.NONE, "MESSAGE", "TRANSFORM"),
                mapping("usage.input_tokens", "usage.input_tokens", "Direct passthrough", MappingLossiness.NONE, "USAGE", "DIRECT"),
                mapping("usage.output_tokens", "usage.output_tokens", "Direct passthrough", MappingLossiness.NONE, "USAGE", "DIRECT"),
                ...
                mapping("usage.cache_creation_input_tokens", "usage.input_tokens_details.cache_write_tokens", "缓存写入 token 路径转换", MappingLossiness.NONE, "USAGE", "RENAME"),
        ...
        map.put(key(ProtocolType.CLAUDE_MESSAGES, ProtocolType.OPENAI_CHAT_COMPLETIONS, ProtocolConversionDirection.REQUEST), List.of(
                ...
                mapping("content[].type=document", "messages[].content[].type=file", "文档块转为 Chat file 内容部件；media_type 写入 data URI 与 filename", MappingLossiness.PARTIAL, "CONTENT_BLOCK", "RESHAPE"),
