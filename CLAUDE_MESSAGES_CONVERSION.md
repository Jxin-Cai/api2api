...
Bedrock 的 thinking/redacted thinking 是受签名保护的响应块：仅在最新用户消息完全由 `tool_result` 组成时，原样回放紧邻的连续 assistant thinking/tool-use 回合；普通用户消息开始新回合时会省略历史 thinking，避免 Bedrock 将旧块误判为被修改的最新 assistant thinking。
