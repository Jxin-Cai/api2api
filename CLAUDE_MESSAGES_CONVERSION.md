...
本文描述 Claude Code 通过 `/v1/messages` 接入时，服务转换到 OpenAI Responses 或 OpenAI Chat Completions 的行为。原则是：可等价映射就转换；不能可靠映射就明确失败，避免静默丢参数导致能力降级。AWS Bedrock 上游使用原生 Claude Messages InvokeModel 协议，不再支持 Converse。
