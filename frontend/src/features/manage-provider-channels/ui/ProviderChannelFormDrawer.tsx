...
export function ProviderChannelFormDrawer({ open, mode, channel = null, onClose, onSaved }: ProviderChannelFormDrawerProps) {
  ...
  async function handlePreviewModels(): Promise<void> {
    try {
      const values = await form.validateFields(['host', 'keyRef']);
      if (!isHttpHost(values.host)) {
        form.setFields([{ name: 'host', errors: ['渠道 Host 必须以 http:// 或 https:// 开头'] }]);
        setPreviewError('渠道 Host 必须以 http:// 或 https:// 开头');
      ...
      if (mode === 'create' && !keyRef) {
        ...
        setPreviewError('预览拉取模型需要填写真实渠道 Key');
      ...
      if (mode === 'edit' && !keyRef && !channel?.hasKey) {
        ...
        setPreviewError('当前渠道没有已保存 Key，请输入真实渠道 Key');
      ...
      if (upstreamProtocols.length === 0) {
        setPreviewError('请先选择至少一个上游调用协议');
      ...
      if (isFormValidationError(error)) {
        setPreviewError('请先填写渠道 Host 和渠道 Key');
        message.warning('请先填写渠道 Host 和渠道 Key');
        return;
      }
      const reason = getApiErrorMessage(error, '请检查 Host、Key 和模型列表权限');
      setPreviewError(reason);
  ...
  async function handleSave(): Promise<void> {
    try {
      ...
      try {
        ...
        message.error(`渠道已保存，但部分模型保存失败：${getApiErrorMessage(error, '请进入模型列表重试')}`);
      }
      ...
      message.error(`保存渠道失败：${getApiErrorMessage(error, '请检查表单内容后重试')}`);
    }
  }
    ...
    >
      ...
      <Space direction="vertical" style={{ width: '100%' }}>
        <Alert
          type="info"
          showIcon
          message="验证并获取模型列表会请求供应商 host/v1/models，并携带 Authorization: Bearer 渠道 Key。"
          description="编辑渠道时可沿用已保存 Key，并保留已配置的优先模型和排序值。默认只勾选已保存的启用模型，新候选需要手动勾选后才会保存为选用模型。"
        />
